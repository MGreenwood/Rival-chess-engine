@echo off
REM Setup script for RivalAI UCI Integration (Windows)
REM This script builds the UCI engine and demonstrates how to use it for training data collection

echo 🏗️  Setting up RivalAI UCI Integration
echo =====================================

REM Check if we're in the right directory
if not exist "engine\Cargo.toml" (
...
if not exist "python\training_games\uci_matches" mkdir python\training_games\uci_matches
if not exist "scripts" mkdir scripts
if not exist "uci_tournament_results" mkdir uci_tournament_results

REM Build the UCI engine
echo 🔨 Building UCI engine...
cd engine
cargo build --release --bin uci
if %ERRORLEVEL% neq 0 (
    echo ❌ Build failed. Check that Rust is installed and try again.
    cd ..
    pause
    exit /b 1
)
...
if not exist "target\release\uci.exe" (
...
python -c "import chess.engine" 2>nul
if %ERRORLEVEL% neq 0 (
...
echo    cd engine
echo    echo uci ^| .\target\release\uci.exe
echo.
echo 2. Run a quick tournament against Stockfish:
echo    python scripts\uci_tournament.py --games 2 --time 1.0
echo.
echo 3. Convert tournament results to training data:
echo    python scripts\convert_uci_games.py uci_tournament_results
echo.
echo 4. Check the training data:
echo    dir python\training_games\uci_matches\
echo.
...
echo   cd engine ^&^& .\target\release\uci.exe
echo   ^> uci
echo   ^> isready
...
echo   python scripts\uci_tournament.py --help
echo.
echo Convert games:
echo   python scripts\convert_uci_games.py --help
echo.
...
echo   python scripts\uci_tournament.py ^
echo     --engines stockfish.exe C:\path\to\lc0.exe ^
...
echo   type uci_tournament_results\tournament_results.json
echo.
...
echo • Ensure opponent engines are in PATH or use full paths
echo • Check python-chess is installed: pip install python-chess
echo • Use .exe extension for Windows engines
echo.
echo For training integration issues:
echo • Check that python\training_games\uci_matches\ exists
